                    ...
                    wishlistItems.filter(item => item && item.product).map((item,index)=>(
                      <Stack component={is480?"":Paper} elevation={1} >
                        ...
                        <Stack paddingLeft={2} paddingRight={2} paddingBottom={2}>
                          ...
                          {/* add to cart */}
                          <Button size={is480?'medium':'large'} variant='contained' sx={{mt:2}} onClick={()=>handleAddToCart(item.product._id)} disabled={item.product ? cartItems.some((cartItem)=>cartItem.product && cartItem.product._id===item.product._id) : true}>{item.product && cartItems.some((cartItem)=>cartItem.product && cartItem.product._id===item.product._id)?'Added to cart':'Add to cart'}</Button>
                        </Stack>
