...
export const ProductDetails = () => {
    ...
    const handleAddRemoveFromWishlist=(e)=>{
        ...
        if(e.target.checked){
            ...
            const index=wishlistItems.findIndex((item)=>item.product && item.product._id===id)
            if (index !== -1 && wishlistItems[index]) {
                dispatch(deleteWishlistItemByIdAsync(wishlistItems[index]._id));
            } else {
                toast.error("Không tìm thấy sản phẩm trong danh sách yêu thích để xóa.");
            }
        }
    }
                ...
                <Stack width={is480?"auto":is1420?"auto":'88rem'} p={is480?2:0} height={is840?"auto":"50rem"} rowGap={5} mt={is840?0:5} justifyContent={'center'} mb={5} flexDirection={is840?"column":"row"} columnGap={is990?"2rem":"5rem"}>
                    ...
                    <Stack rowGap={"1.5rem"} width={is480?"100%":'25rem'}>
                        ...
                        <Stack rowGap={".5rem"}>
                            ...
                            {/* description */}
                            {product?.description && <Typography variant='body1' color="text.secondary">{product?.description}</Typography>}
                            ...
                            {/* price */}
                            <Typography variant='h5'>{new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(product?.price)}</Typography>
                        </Stack>
                        ...
                        <Stack rowGap={".8rem"}>
                            <Stack flexDirection={'row'} alignItems={'center'} columnGap={'1rem'}>
                                ...
                                <Typography>Tên nhân vật: {product?.name}</Typography>
                            </Stack>
                            <Stack flexDirection={'row'} alignItems={'center'} columnGap={'1rem'}>
                                <CheckIcon sx={{color:'green'}}/>
                                <Typography>Manga - Anime: {product?.anime}</Typography>
                            </Stack>
                            <Stack flexDirection={'row'} alignItems={'center'} columnGap={'1rem'}>
                                <CheckIcon sx={{color:'green'}}/>
                                <Typography>Chiều cao: {product?.height} cm</Typography>
                            </Stack>
                            <Stack flexDirection={'row'} alignItems={'center'} columnGap={'1rem'}>
                                <CheckIcon sx={{color:'green'}}/>
                                <Typography>Chiều ngang: {product?.weight} cm</Typography>
                            </Stack>
