...
export const Checkout = () => {
    ...
    const orderTotal=cartItems.reduce((acc,item)=>(item.product ? item.product.price * item.quantity : 0)+acc,0)
            ...
            <Stack rowGap={3}>
                    ...
                    <Stack rowGap={2}>
                        ...
                        <Stack flexDirection={'row'} justifyContent={'flex-start'} alignItems={'center'}>
                            ...
                            <Typography>Tiền mặt khi nhận hàng</Typography>
                        </Stack>

                        <Stack flexDirection={'row'} justifyContent={'flex-start'} alignItems={'center'}>
                            <Radio value={'BANK_TRANSFER'} name='paymentMethod' checked={selectedPaymentMethod==='BANK_TRANSFER'} onChange={()=>setSelectedPaymentMethod('BANK_TRANSFER')}/>
                    ...
                    {selectedPaymentMethod === 'BANK_TRANSFER' && (
                        <Paper elevation={3} sx={{ p: 2, mt: 2 }}>
                            <Typography variant="h6" gutterBottom>Thông tin chuyển khoản</Typography>
                            <Typography>Ngân hàng: VCB</Typography>
                            <Typography>Số tài khoản: 1025950951</Typography>
                            <Typography>Tên tài khoản: NGUYEN VAN A (Ví dụ)</Typography>
                            <Box sx={{ mt: 2, display: 'flex', justifyContent: 'center' }}>
                                <img src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=VCB_1025950951_${orderTotal+SHIPPING+TAXES}`} alt="QR Code" style={{ maxWidth: '100%', height: 'auto' }} />
                            </Box>
                            <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>
                                Vui lòng quét mã QR hoặc chuyển khoản vào tài khoản trên với nội dung là mã đơn hàng. Đơn hàng của bạn sẽ được xác nhận sau khi chúng tôi nhận được thanh toán.
                            </Typography>
                        </Paper>
                    )}
